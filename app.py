...
- If the target has a neighbour close enough in azimuth that direction alone
  cannot separate them (measured by analyze_angular_safety.py), the app
  automatically switches to the close-speaker path: beamform, then blind-
  separate with SepFormer (2- or 3-speaker), then identify the target stream
  with ECAPA-TDNN speaker embeddings. See fallback_separation.py.
...
@st.cache_resource(show_spinner="Loading separation + speaker-ID models...")
def load_fallback():
    """SepFormer (2/3-speaker) + ECAPA-TDNN, for the close-speaker path."""
...
def run_extraction(model, mic_positions, signals, rel_positions, target_idx,
                   fallback=None, use_enrolment=True):
    """
    Full pipeline: sim -> crop -> normalize -> steer -> model -> metrics.

    When the target has a neighbour inside the measured unsafe cone and
    `fallback` is provided, the beamformed output is additionally passed
    through blind separation + ECAPA-TDNN identification, and that result
    becomes the reported output. See fallback_separation.py.
    """
    ...
    # ---- close-speaker fallback (only when the geometry calls for it) ----
    az_all = [azimuth_of(px, py) for px, py in rel_positions]
    geo = fbs.analyze_geometry(az_all, int(target_idx))
    fb_result, sisdr_fb = None, None
    final, path = extracted, "direct"

    if fallback is not None and geo["needs_fallback"]:
        enrolment = signals[int(target_idx)] if use_enrolment else None
        fb_result = fallback.run(extracted, n_cluster=geo["n_cluster"],
                                 reference=enrolment)
        fb_wave = fb_result["waveform"]
        sisdr_fb = tr.si_sdr_db(torch.tensor(fb_wave).unsqueeze(0), tgt_t).item()
        final, path = fb_wave, "fallback"

    return {
        "mixture_ref": mixture[tr.REFERENCE_MIC],
        "extracted": extracted,          # direction-conditioned model output
        "final": final,                  # what the app reports as THE answer
        "path": path,
        ...
        "sisdr_ext": sisdr_ext,
        "sisdr_fb": sisdr_fb,
        "sisdr_final": sisdr_fb if sisdr_fb is not None else sisdr_ext,
    }
...
def main():
        ...
        for j in range(i + 1, len(speakers)):
            ...
            if sep < MIN_ANGULAR_SEP_DEG:
                warnings.append(f"**{speakers[i]['name']}** and **{speakers[j]['name']}** "
                                f"are only {sep:.1f}° apart — closer than the "
                                f"{MIN_ANGULAR_SEP_DEG:.0f}° minimum the model was trained on.")
    ...
    with col_run:
        ...
        # ---- close-speaker fallback status for the CURRENT target ----
        az_all = [azimuth_of(s["x"], s["y"]) for s in speakers]
        geo = fbs.analyze_geometry(az_all, int(target_idx))
        use_fallback = st.checkbox(
            "Close-speaker fallback", value=True,
            help="When a neighbour is inside the measured unsafe cone, the "
                 "beamformed signal is passed through SepFormer (2- or 3-speaker) "
                 "and the target stream is identified with ECAPA-TDNN. Off = the "
                 "direction-conditioned model alone.")
        use_enrolment = st.checkbox(
            "Identify using the speaker's own voice clip", value=True,
            help="ECAPA compares the separated streams against this speaker's "
                 "enrolment clip. Off = compare against the beamformed signal "
                 "itself, which is less reliable below ~6° separation.")

        if geo["needs_fallback"]:
            near = speakers[geo["nearest_idx"]]["name"]
            st.info(
                f"**Close-speaker geometry.** Nearest neighbour (**{near}**) is "
                f"{geo['nearest_sep_deg']:.1f}° away — inside the "
                f"{geo['trigger_deg']:.1f}° trigger for a room with "
                f"{geo['n_speakers']} speakers. "
                + (f"The {geo['n_cluster']}-speaker separation pipeline will run "
                   f"after beamforming." if use_fallback
                   else "Fallback is switched off — expect degraded output.")
            )
        else:
            st.success(
                f"**Safe geometry.** Nearest neighbour is "
                f"{geo['nearest_sep_deg']:.1f}° away, outside the "
                f"{geo['trigger_deg']:.1f}° trigger for {geo['n_speakers']} "
                f"speakers — the direction-conditioned model handles this alone.")
    ...
    if run:
        with st.spinner("Preparing voices, simulating the room, running the model..."):
            ...
            fb = load_fallback() if (use_fallback and geo["needs_fallback"]) else None
            result = run_extraction(model, mic_positions, signals,
                                    rel_positions, int(target_idx),
                                    fallback=fb, use_enrolment=use_enrolment)
    ...
    if "results" in st.session_state:
        ...
        used_fb = res["path"] == "fallback"
        m1, m2, m3, m4 = st.columns(4)
        m1.metric("SI-SDR in mixture", f"{res['sisdr_mix']:.2f} dB")
        m2.metric("Beamformer only", f"{res['sisdr_ext']:.2f} dB",
                  delta=f"{res['sisdr_ext'] - res['sisdr_mix']:+.2f} dB")
        if used_fb:
            m3.metric("After separation", f"{res['sisdr_fb']:.2f} dB",
                      delta=f"{res['sisdr_fb'] - res['sisdr_ext']:+.2f} dB vs beamformer")
        else:
            m3.metric("After separation", "not needed")
        m4.metric("Nearest neighbour", f"{res['geometry']['nearest_sep_deg']:.1f}°")

        st.caption(
            f"Path used: **{'beamforming → SepFormer → ECAPA-TDNN' if used_fb else 'direction-conditioned model only'}**"
            + (f" · {res['fallback']['separator']} · reference: "
               f"{res['fallback']['reference_kind']}" if used_fb else ""))

        signals_to_play = [res["mixture_ref"], res["extracted"], res["target_ref"]]
        if used_fb:
            signals_to_play.insert(2, res["final"])
        played = playback_group(*signals_to_play)

        cols = st.columns(len(played))
        labels = ["**Mixture** (reference mic)", "**Beamformer output** (model only)"]
        if used_fb:
            labels.append("**Final output** (after separation + ID)")
        labels.append("**Clean target** (ground truth)")
        for col, label, sig in zip(cols, labels, played):
            with col:
                st.markdown(label)
        ...
        if used_fb:
            fbres = res["fallback"]
            with st.expander("Separated streams and speaker-ID scores", expanded=True):
                st.markdown(
                    f"The beamformed signal was split into {len(fbres['streams'])} "
                    f"streams; ECAPA-TDNN compared each against the "
                    f"{'target speaker&rsquo;s enrolment clip' if fbres['reference_kind'] == 'enrolment' else 'beamformed signal'}. "
                    f"Stream {fbres['chosen'] + 1} won by "
                    f"{fbres['margin']:.3f} cosine similarity.",
                    unsafe_allow_html=True)
                scols = st.columns(len(fbres["streams"]))
                streams_p = playback_group(*fbres["streams"])
                for k, (col, sig) in enumerate(zip(scols, streams_p)):
                    with col:
                        mark = " ✅ **reported as target**" if k == fbres["chosen"] else ""
                        st.markdown(f"Stream {k + 1} — similarity "
                                    f"`{fbres['similarities'][k]:+.3f}`{mark}")
                        st.audio(sig, sample_rate=FS)
